...
class TensorboardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.training_env.env_method("check_if_done", indices=[0])[0]:
            ...
            # explore_map and current_event_flags_set are v2-only attrs;
            # baselines RedGymEnv uses KNN and does not have them.
            # get_attr() runs in a subprocess — a missing attr crashes the
            # worker before it can send back an AttributeError, so the main
            # process sees EOFError/BrokenPipeError, not AttributeError.
            # These blocks are removed rather than wrapped.
            #
            # explore_map:
